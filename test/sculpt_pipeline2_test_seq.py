...
A_OD1 = Atom(chain='A', residue=99, name='OE1')
A_OD2 = Atom(chain='A', residue=99, name='OE2')
A_CG =  Atom(chain='A', residue=99, name='CD')
...
input_structures_dir = Path('./run_data/data/1OHP_monomer/')
...
standard_molecules = [StandardMolecule(structure_file=sdf_file) for sdf_file in [ligand_reference]]

new_design = Design(name='test')
new_design.load_structure(Path('run_data') / 'fuzzy_mira_ithink_h.cif')
new_design.structure.standardize(standard_molecules=standard_molecules, use_hydrogens=True, renumber=True)
new_design.load_sequence_from_structure()
# Add hydrogens
# hydrogen_adder = SculptHydrogenAdder()
# new_design = hydrogen_adder.add_hydrogens(new_design)
print(new_design.structure)
new_design.structure_file('test.cif')

folder = SculptFolder(model='Chai-1', add_hydrogens=True)
desig_folded = folder(new_design)

# Test scoring function:
print(scoring_function.score(desig_folded))
